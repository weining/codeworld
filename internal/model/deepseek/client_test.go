...
func TestStreamSendsStreamingRequestAndEmitsDeltasUsageAndDone(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		_, _ = w.Write([]byte("data: {\"usage\":{\"prompt_tokens\":4,\"completion_tokens\":2,\"total_tokens\":6,\"prompt_cache_hit_tokens\":1},\"choices\":[{\"delta\":{}}]}\n\n"))
	...
	if events[2].Kind != model.StreamEventUsage || events[2].Usage.CacheTokens != 1 || events[2].Usage.TotalTokens != 6 {
		t.Fatalf("usage event = %#v, want deepseek usage", events[2])
	}
